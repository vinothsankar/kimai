...
require_once __DIR__.\DIRECTORY_SEPARATOR.'FirewallConfig'.\DIRECTORY_SEPARATOR.'KimaiLdapConfig.php';
...
class FirewallConfig 
{
    ...
    public function kimaiLdap(array $value = []): \Symfony\Config\Security\FirewallConfig\KimaiLdapConfig
    {
        if (null === $this->kimaiLdap) {
            $this->_usedProperties['kimaiLdap'] = true;
            $this->kimaiLdap = new \Symfony\Config\Security\FirewallConfig\KimaiLdapConfig($value);
        } elseif (0 < \func_num_args()) {
            throw new InvalidConfigurationException('The node created by "kimaiLdap()" has already been initialized. You cannot pass values the second time you call kimaiLdap().');
        }

        return $this->kimaiLdap;
    }
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('kimai_ldap', $value)) {
            $this->_usedProperties['kimaiLdap'] = true;
            $this->kimaiLdap = new \Symfony\Config\Security\FirewallConfig\KimaiLdapConfig($value['kimai_ldap']);
            unset($value['kimai_ldap']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['kimaiLdap'])) {
            $output['kimai_ldap'] = $this->kimaiLdap->toArray();
        }
